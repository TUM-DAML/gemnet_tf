...
class InteractionBlock(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.dense_ca = Dense(
            emb_size_edge, activation=activation, use_bias=False, name="dense_ca"
        )
        ...
        self.concat_layer = EdgeEmbedding(
            emb_size_edge, activation=activation, name="concat"
        )
        self.residual_m = [
        ...
        self.inv_sqrt_2 = tf.cast(tf.constant(1 / tf.sqrt(2.0)), dtype=tf.float32)
        self.inv_sqrt_3 = tf.cast(tf.constant(1 / tf.sqrt(3.0)), dtype=tf.float32)
    ...
    def call(self, inputs):
        ...
        ## --------------------------------------- Update Atom Embeddings ---------------------------------------- ##
        h2 = self.atom_update((h, m, rbfh, id_a))
...
class InteractionBlockTripletsOnly(tf.keras.layers.Layer):
    """
    ...
    def __init__(
        ...
        emb_size_rbf,
        emb_size_cbf,
        emb_size_sbf,  # not used
        emb_size_bil_trip,
        emb_size_bil_quad,  # not used
        num_before_skip,
        num_after_skip,
        num_concat,
        num_atom,
    ...
    def call(self, inputs):
        ...
        ## ---------------------------------------- Update Atom Embeddings --------------------------------------- ##
        h2 = self.atom_update((h, m, rbfh, id_a))  # (nAtoms, emb_size_atom)
...
class QuadrupletInteraction(tf.keras.layers.Layer):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.down_projection = Dense(
            emb_size_quad, activation=activation, use_bias=False, name="dense_down"
        )
        self.up_projection_ca = Dense(
            emb_size_edge, activation=activation, use_bias=False, name="dense_up_ca"
        )
        self.up_projection_ac = Dense(
            emb_size_edge, activation=activation, use_bias=False, name="dense_up_ac"
        )
    ...
    def call(self, inputs):
        ...
        x_db = tf.gather(x_db, id4_expand_intm_db)  # (intmTriplets, emb_size_quad)

        x_db2 = x_db * self.mlp_cbf(cbf)  # (intmTriplets, emb_size_quad)
        x_db = self.scale_cbf(x_db, x_db2)

        # Transform via spherical bessel basis
        x_db = tf.gather(x_db, id4_expand_abd)  # (nQuadruplets, emb_size_quad)
        x = self.mlp_sbf((sbf, x_db, id4_reduce_ca, Kidx, nEdges))  # (nEdges, emb_size_bilinear)
        ...
        x_ac = tf.gather(x_ac, id_swap)  # swap to add to edge a->c and not c->a
        x4 = x_ca + x_ac
...
class TripletInteraction(tf.keras.layers.Layer):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Dense transformation
        self.dense_ba = Dense(
            emb_size_edge, activation=activation, use_bias=False, name="dense_ba"
        )

        # Up projections of basis representations, bilinear layer and scaling factors
        self.mlp_rbf = Dense(
            emb_size_edge, activation=None, name="MLP_rbf3_2", use_bias=False
        )
        self.scale_rbf = ScalingFactor(scale_file=scale_file, name=name + "_had_rbf")

        self.mlp_cbf = EfficientInteractionBilinear(
            emb_size_bilinear, name="MLP_cbf3_2"
        )
        ...
        # Down and up projections
        self.down_projection = Dense(
            emb_size_trip, activation=activation, use_bias=False, name="dense_down"
        )
        self.up_projection_ca = Dense(
            emb_size_edge, activation=activation, use_bias=False, name="dense_up_ca"
        )
    ...
    def call(self, inputs):
        ...
        -------
            m: Tensor, shape=(nEdges, emb_size_edge)
                Edge embeddings (c->a).
        """
        m, rbf3, cbf3, Kidx3, id_swap, id3_expand_ba, id3_reduce_ca = inputs
        nEdges = tf.shape(m)[0]
        ...
        x_ba = tf.gather(x_ba, id3_expand_ba)

        # Efficient bilinear layer
        x = self.mlp_cbf((cbf3, x_ba, id3_reduce_ca, Kidx3, nEdges))  # (nEdges, emb_size_bilinear)

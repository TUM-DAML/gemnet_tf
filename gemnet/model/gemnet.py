...
class GemNet(tf.keras.Model):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not self.triplets_only:
            self.mlp_rbf4 = Dense(
                emb_size_rbf, activation=None, name="MLP_rbf4_shared", use_bias=False
            )
            self.mlp_cbf4 = Dense(
                emb_size_cbf, activation=None, name="MLP_cbf4_shared", use_bias=False
            )
            self.mlp_sbf4 = EfficientInteractionDownProjection(
                emb_size_sbf, name="MLP_sbf4_shared"
            )

        self.mlp_rbf3 = Dense(
            emb_size_rbf, activation=None, name="MLP_rbf3_shared", use_bias=False
        )
        self.mlp_cbf3 = EfficientInteractionDownProjection(
            emb_size_cbf, name="MLP_cbf3_shared"
        )

        # Share the dense Layer of the atom embedding block accross the interaction blocks
        self.mlp_rbf_h = Dense(
            emb_size_rbf, activation=None, name="MLP_rbfh_shared", use_bias=False
        )
        self.mlp_rbf_out = Dense(
            emb_size_rbf, activation=None, name="MLP_rbfout_shared", use_bias=False
        )
        ### ------------------------------------------------------------------------------------- ###

        # Embedding block
        self.atom_emb = AtomEmbedding(emb_size_atom)
        self.edge_emb = EdgeEmbedding(emb_size_edge, activation=activation)
    ...
    def calculate_interatomic_vectors(R, id_s, id_t):
        ...
        Rs = tf.gather(R, id_s)
        V_st = Rt - Rs  # s -> t
        D_st = tf.sqrt(tf.reduce_sum(V_st ** 2, -1))
        V_st /= D_st[..., None]
        return D_st, V_st
    ...
    def calculate_neighbor_angles(R_ac, R_ab):
        ...
        """
        # cos(alpha) = (u * v) / (|u|*|v|)
        x = tf.reduce_sum(R_ac * R_ab, axis=-1)  # shape = (N,)
        # sin(alpha) = |u x v| / (|u|*|v|)
        y = tf.linalg.cross(R_ac, R_ab)  # shape = (N,3)
        # avoid that for y == (0,0,0) the gradient wrt. y becomes NaN
        y = tf.norm(y, axis=-1) # shape = (N,)
        y = tf.maximum(y, 1e-9)  
    ...
    def normalize(R_ac):
        ...
        Returns
        -------
            R_ac_normed: Tensor, shape = (N,)
                Unit vector from atom a to c.
        """
        R_ac_normalized, _ = tf.linalg.normalize( R_ac, ord=2, axis=-1)
        return R_ac_normalized
    ...
    def vector_rejection(R_ab, P_n):
        ...
        """
        a_x_b = tf.reduce_sum(R_ab * P_n, axis=-1)
        b_x_b = tf.reduce_sum(P_n * P_n, axis=-1)
    ...
    def calculate_angles(
        ...
        """
        # # ---------------------------------- a - b <- d ---------------------------------- #
        Ra = tf.gather(R, tf.gather(id4_int_a, id4_expand_intm_ab))  # a       (intmTriplets,3)
        Rb = tf.gather(R, tf.gather(id4_int_b, id4_expand_intm_ab))  # a       (intmTriplets,3)
        # Rb = tf.gather(tf.gather(R, id_a), id4_expand_intm_db)     # b       (intmTriplets,3)
        Rd = tf.gather(R, tf.gather(id_c, id4_expand_intm_db))  # d       (intmTriplets,3)
        ...
        R_bd_proj = tf.gather(R_bd_proj, id4_expand_abd)  # (nQuadruplets,)

        # --------------------------------- c -> a <- b ---------------------------------- #
        Rc = tf.gather(R, tf.gather(id_c, id4_reduce_intm_ca))  # c      (intmTriplets,3)
        Ra = tf.gather(R, tf.gather(id_a, id4_reduce_intm_ca))  # a      (intmTriplets,3)
        # Ra = tf.gather(tf.gather(R, id4_int_a), id4_reduce_intm_ab) # a      (intmTriplets,3)
        Rb = tf.gather(R, tf.gather(id4_int_b, id4_reduce_intm_ab))  # b      (intmTriplets,3)

        R_ac = Rc - Ra  # (intmTriplets,3)
        R_ab = Rb - Ra  # (intmTriplets,3)
        angle_cab = GemNet.calculate_neighbor_angles(R_ab, R_ac)  # (intmTriplets,)
        angle_cab = tf.gather(angle_cab, id4_reduce_cab)  # (nQuadruplets,)

        # project for calculating dihedral angle
        R_ac_proj = GemNet.vector_rejection(R_ac, R_ab)  # c |- a - b
        R_ac_proj = tf.gather(R_ac_proj, id4_reduce_cab)  # (nQuadruplets,)
    ...
    def calculate_angles3(R, id_c, id_a, id3_reduce_ca, id3_expand_ba):
        ...
        -------
            angle_cab: Tensor, shape = (nTriplets,)
                Angle between atoms c <- a -> b.
        """
        Rc = tf.gather(R, tf.gather(id_c, id3_reduce_ca))
        Ra = tf.gather(R, tf.gather(id_a, id3_reduce_ca))
        Rb = tf.gather(R, tf.gather(id_c, id3_expand_ba))
    ...
    def call(self, inputs):
        ...
        if not self.direct_forces:
            with tf.GradientTape(watch_accessed_variables=False) as force_tape:
                force_tape.watch(inputs["R"])
                energy, _ = self.subcall(inputs)
            forces = -force_tape.gradient(energy, inputs["R"])
            forces = tf.convert_to_tensor(forces)
        else:
            energy, forces = self.subcall(inputs)
        return energy, forces

    def subcall(self, inputs):
        ...
        if self.extensive:
            E_a = tf.math.segment_sum(E_a, batch_seg)  # (nMolecules, num_targets)
        else:
            E_a = tf.math.segment_mean(E_a, batch_seg)  # (nMolecules, num_targets)

        if self.direct_forces:
            nAtoms = tf.shape(Z)[0]
            if self.forces_coupled:  # enforce F_ac = F_ca
                nEdges = tf.shape(id_c)[0]
                F_ca = tf.math.unsorted_segment_sum(
                    F_ca, id_undir, tf.cast(nEdges / 2, dtype=tf.int32)
                )  # (nEdges/2, num_targets)
                F_ca = F_ca / 2  # mean of edges
                F_ca = tf.gather(F_ca, id_undir)  # (nEdges, num_targets)

            # map forces in edge directions
            F_ji = F_ca[:, :, None] * V_ca[:, None, :]  # (nEdges, num_targets, 3)
            F_j = tf.math.unsorted_segment_sum(
                F_ji, id_a, nAtoms

...
class DataContainer:
    ...
    # keys that are used for indexing (int)
    def __init__(
        self,
        path,
        cutoff,
    ...
    def __getitem__(self, idx):
        ...
        if not self.triplets_only:
            idx_data["id4_int_a"] = idx_int_t.astype("int32")
            idx_data["id4_int_b"] = idx_int_s.astype("int32")
        ...
        # id3_reduce_ca is for b -> a -> c but we want c -> a <- b
        ...
        # get quadruplets
        # embed msg from c -> a with all quadruplets for d and b: c -> a - b <- d
    ...
    def convert_to_tensor(self, data):
        for key in data:
            data[key] = tf.constant(data[key], dtype=self.dtypes[key])
        return data
    ...
    def get_dtypes(self):
        ...
        dtypes_input = {}
        if self.addID:
            dtypes_input["id"] = tf.int32
        dtypes_input["Z"] = tf.int32
        dtypes_input["N"] = tf.int32
        dtypes_input["R"] = tf.float32
        for key in self.index_keys:
            dtypes_input[key] = tf.int32

        dtypes_target = {}
        dtypes_target["E"] = tf.float32
        dtypes_target["F"] = tf.float32
    ...
    def get_shapes(self):
        ...
        -------
            dtypes: tuple
                (shapes_input, shapes_target) Shapes of the inputs and targets
                stored in dicts.
        """
        # Shapes of dataset values
        shapes_input = {}
        if self.addID:
            shapes_input["id"] = [None]
        shapes_input["Z"] = [None]
        shapes_input["N"] = [None]
        ...
        shapes_target = {}
        shapes_target["E"] = [None, 1]
        shapes_target["F"] = [None, 3]

        return shapes_input, shapes_target
